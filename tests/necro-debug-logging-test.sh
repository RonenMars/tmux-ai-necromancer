...
NECROMANCER_DEBUG=off bash "$ROOT/scripts/necro-clean-debug-logs.sh" --dry-run | grep -q 'Would remove 2 debug log file'
[ -f "$LOG" ] || { echo "dry-run removed a log" >&2; exit 1; }
NECROMANCER_DEBUG=off bash "$ROOT/scripts/necro-clean-debug-logs.sh" | grep -q 'Removed 2 debug log file'
